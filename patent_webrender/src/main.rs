use std::{fs, io, ops::{Add, Mul, Sub}, usize};
		...
		let random_numbers: Vec<Task> = (0..count)
			.map(|_| {
				match rng.random_range(0..3){
					1=>Task::new(rng.random_range(4..5), TaskNew::UploadTask(rng.random_range(1..3))),
					2=>Task::new(rng.random_range(200..300),TaskNew::RenderTask(rng.random_range(0..10))),
...
trait AsVector: Sized {
    ...
    fn add<E: Add<Output = E> + Mul<Output = E> + Copy, const N: usize>(&self, rhs: &Self, scalar: E) -> Self
    where
...
impl Task{
	...
	fn is_running(&self, time: usize)->bool{
		self.time_in.is_some_and(|v| v<=time && time<v+self.time_span)
	}
...
impl Server{
		...
		loop{
			...
			while{
				...
				for i in v.iter_mut().filter(|v| v.is_pending()){
					if (capacity.clone()-i.resource.clone()).as_vector().iter().all(|v| *v>=0){
						i.run(t);
			...
			// 最初に終わる時刻
			let t1=v.iter().filter(|v| v.is_running(t)).map(|i| i.time_span+i.time_in.unwrap()).reduce(|a,b| a.min(b)).unwrap_or(usize::MAX);
			println!("{t}->{t1}");
			t=t1;
	...
	pub fn visualize<'a>(&self, mut w: impl io::Write, x: impl Iterator<Item=&'a Task>+Clone)->io::Result<()>{
		for i in x.clone().enumerate(){
			...
			let used=x.clone().filter_map(|v| v.is_running(t).then_some(v.resource.clone())).reduce(|a,b| a+b).unwrap_or_default();
			writeln!(
				w, 
				"{t} {} {} {} {} {}", 
				i.1.time_span,
				used.vram_gbytes,
				used.network_upload_gbps,
				stage,
			 	i.1.name
			)?
		}
